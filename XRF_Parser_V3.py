...
st.set_page_config(page_title="SPX_Parser_V3.py", layout="wide")
st.title("SPX_Parser_V3.py")

with st.sidebar:
    st.header("Load file(s)")
    use_default = st.checkbox("Use default sample (data/sample.spx)", value=True)
    uploaded_files = st.file_uploader("...or upload one or more .spx", type=["spx"], accept_multiple_files=True)
    ...
    log_scale = st.checkbox("Logarithmic Y scale", value=False)
    x_range = st.slider("Energy range (keV)", 0.0, 40.0, (0.0, 20.0), step=0.1)
...
if uploaded_files:
    for uf in uploaded_files:
        with tempfile.NamedTemporaryFile(delete=False, suffix=".spx") as tmp:
            ...
            path = tmp.name
        try:
            rec = parse_spx_file(path)
            rec["source_name"] = uf.name
            records.append(rec)
        except Exception as e:
            st.error(f"Failed to parse {uf.name}: {e}")
elif use_default:
    try:
        rec = parse_spx_file(default_path)
        rec["source_name"] = Path(default_path).name
        records.append(rec)
    except Exception as e:
        st.error(f"Failed to load default sample: {e}")
        st.stop()
else:
    st.info("Please upload at least one .spx file or use the default sample.")
    st.stop()
...
all_names = [rec.get("source_name", f"Spectrum {i+1}") for i, rec in enumerate(records)]

selected_names = st.multiselect(
    "Select spectra to display",
    options=all_names,
    default=all_names,  # all selected by default
)
...
if show_plot:
    ...
    for i, rec in enumerate(records):
        ...
        ax.plot(x, counts, lw=1, color=colors[i % len(colors)], label=rec.get("source_name", f"Spectrum {i+1}"))

    ax.set_xlim(x_range)
    ax.set_xlabel("Energy (keV)")
    ax.set_ylabel("Counts")
    ax.set_title("SPX Spectra")
    if log_scale:
        ax.set_yscale("log")
...
tab_mix, tab_exp = st.tabs(["🔬 Synthetic Mixture Laboratory",
                            "🧪 Experimental Spectrum Decomposition"])
...
def select_roi(comp_matrix, roi_fraction):
    """
    comp_matrix: shape (n_components, n_channels)
    Select ROI based on contrast between component spectra.
    Works for 1, 2, or 3 spectra.
    """

    n_comp = comp_matrix.shape[0]

    # Case 1: only ONE reference spectrum → no contrast possible
    if n_comp == 1:
        # Use all channels (ROI = full spectrum)
        return np.ones(comp_matrix.shape[1], dtype=bool)

    # Case 2: TWO reference spectra
    if n_comp == 2:
        diff = np.abs(comp_matrix[0] - comp_matrix[1])

    # Case 3: THREE reference spectra
    elif n_comp == 3:
        diff = (
            np.abs(comp_matrix[0] - comp_matrix[1]) +
            np.abs(comp_matrix[0] - comp_matrix[2]) +
            np.abs(comp_matrix[1] - comp_matrix[2])
        )

    # Threshold by percentile
    threshold = np.percentile(diff, 100 - roi_fraction)
...
def run_nnls_with_bootstrap(A, y, noise_level, n_boot):
    """
    A: (n_channels, n_components)
    y: mixture (n_channels,)
    """
    # Base NNLS
    coeffs, _ = nnls(A, y)
    ...
    if n_boot > 0:
        ...
        for _ in range(n_boot):
            noisy = y + noise_level * np.random.normal(0, sigma)
        ...
        mean = coeffs
        lower = upper = np.zeros_like(coeffs)

    return coeffs, mean, lower, upper

with tab_mix:
    st.header("Synthetic Mixture Laboratory")

    # Select reference spectra
    ref_names = [r["source_name"] for r in records]

    selected_refs = st.multiselect(
        "Select 2 or 3 reference spectra to mix:",
        options=ref_names,
        default=ref_names[:2],
        max_selections=3
    )
    if len(selected_refs) < 1:
        st.info("Select at least one reference spectrum.")
        st.stop()

    refs = [next(r for r in records if r["source_name"] == nm)
            for nm in selected_refs]

    # Choose weights
    st.subheader("Mixing Fractions")
    cols = st.columns(len(refs))
    weights = []
    for col, r in zip(cols, refs):
        with col:
            w = st.slider(f"Fraction of {r['source_name']}", 0.0, 1.0, 1/len(refs), 0.01)
            weights.append(w)
    weights = np.array(weights)
    weights = weights / weights.sum()

    # Construct matrix
    comp_matrix = np.array([np.array(r["counts"], float) for r in refs])
    min_len = min(c.size for c in comp_matrix)
    comp_matrix = comp_matrix[:, :min_len]
    synthetic = np.sum(weights[:, None] * comp_matrix, axis=0)

    # Noise
    noise_level = st.slider("Noise level", 0.0, 2.0, 0.0, 0.05)
    if noise_level > 0:
        sigma = np.sqrt(np.maximum(synthetic, 1))
        synthetic = synthetic + noise_level * np.random.normal(0, sigma)
        synthetic = np.clip(synthetic, 0, None)

    # ROI
    st.subheader("ROI Selection")
    roi_fraction = st.slider("Top X% highest-contrast channels", 5, 100, 100, step=5)
    roi_mask = select_roi(comp_matrix, roi_fraction)

    A = comp_matrix[:, roi_mask].T
    y = synthetic[roi_mask]
    x_roi = energy_axis_keV(refs[0])[:min_len][roi_mask]

    # NNLS + bootstrap
    st.subheader("Bootstrap Settings")
    do_boot = st.checkbox("Enable bootstrap CIs", True)
    n_boot = st.number_input(
        ...
        value=300,
        step=50,
        key="bootstrap_samples_synthetic",
    ) if do_boot else 0

    coeffs, mean, lower, upper = run_nnls_with_bootstrap(A, y, noise_level, n_boot)

    # Fit curve
    fit_curve = A @ coeffs
    r2 = r2_score(y, fit_curve)

    # Plot
    fig, ax = plt.subplots(figsize=(9,4))
    ax.plot(x_roi, y, label="Synthetic mixture", color="black")
    ax.plot(x_roi, fit_curve, label=f"NNLS fit (R²={r2:.4f})")
    ax.legend(); ax.grid(); ax.set_xlabel("Energy (keV)")
    ax.set_xlim(x_range)
    if log_scale:
        ax.set_yscale("log")
    st.pyplot(fig, clear_figure=True)
    ...
    df_ci = pd.DataFrame({
        "Component": selected_refs,
        "True_fraction": weights,
        "NNLS_fraction": coeffs,
        "Bootstrap_mean": mean,
        "CI_lower": lower,
        "CI_upper": upper,
    })
    ...
    format_dict = {
        col: "{:.4f}" for col in df_ci.columns
        if pd.api.types.is_numeric_dtype(df_ci[col])
    }

    st.table(df_ci.style.format(format_dict))

with tab_exp:
    st.header("Experimental Spectrum Decomposition")

    # ---- Upload ----
    st.subheader("Upload Experimental Spectrum (.spx)")
    exp_file = st.file_uploader("Experimental .spx file", type=["spx"], key="exp_spx")

    if exp_file is None:
        st.info("Please upload an experimental spectrum.")
        st.stop()

    # Parse
    with tempfile.NamedTemporaryFile(delete=False, suffix=".spx") as tmp:
        tmp.write(exp_file.getvalue())
        exp_path = tmp.name

    exp_rec = parse_spx_file(exp_path)
    exp_counts = np.array(exp_rec["counts"], float)
    exp_x = energy_axis_keV(exp_rec)

    # ---- Reference selection ----
    st.subheader("Reference spectra for decomposition")
    ref_names = [r["source_name"] for r in records]

    selected_refs = st.multiselect(
        "Choose 1–3 reference spectra:",
        options=ref_names,
        default=ref_names[:1],
        max_selections=3,
        key="ref_select_exp",
    ...
    # ---- Align spectra ----
    comp_matrix = np.array([np.array(r["counts"], float) for r in refs])
    min_len = min([exp_counts.size] + [c.size for c in comp_matrix])
    comp_matrix = comp_matrix[:, :min_len]
    exp_counts = exp_counts[:min_len]
    exp_x = exp_x[:min_len]

    # ---- Plot experimental + reference spectra ----
    st.subheader("Experimental + Reference Spectra (normalized)")
    fig0, ax0 = plt.subplots(figsize=(9,4))

    ax0.plot(exp_x, normalize(exp_counts), label="Experimental", color="black")

    for r, c in zip(refs, comp_matrix):
        ax0.plot(exp_x, normalize(c), label=f"Ref: {r['source_name']}")
    ...
    # ---- ROI ----
    st.subheader("ROI Selection")
    roi_fraction = st.slider("Keep top X% contrast channels", 5, 100, 100, step=5,
                             key="roi_fraction_exp")

    roi_mask = select_roi(comp_matrix, roi_fraction)

    A = comp_matrix[:, roi_mask].T  # shape (n_channels_roi × n_comp)
    y = exp_counts[roi_mask]
    x_roi = exp_x[roi_mask]

    # ---- Bootstrap settings ----
    st.subheader("Bootstrap Settings")
    do_boot = st.checkbox("Enable bootstrap CI", True, key="enable_boot_exp")
    ...
    n_boot = (
        st.number_input(
            ...
            value=300,
            step=50,
            key="bootstrap_samples_exp",
        ) if do_boot else 0
    )

    noise_level = st.slider("Bootstrap noise level", 0.0, 2.0, 0.1, 0.05,
                            key="bootstrap_noise_exp")
    ...
    # --------------------------------------------------------
    st.subheader("NNLS Fit and Residuals")
    ...
    # MAIN PLOT
    ax1.plot(x_roi, normalize(y), 'o', label="Experimental (ROI)")
    ax1.plot(x_roi, normalize(fit_curve), label=f"NNLS fit (R²={r2:.4f})")
    ...
    ax2.plot(x_roi, abs(residuals), color="red")
